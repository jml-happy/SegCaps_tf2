...
def main(args):
    ...
    # get all images containing given categories with only a single object
    catIds = coco.getCatIds(catNms=category_list);
    annIds = coco.getAnnIds(imgIds=None, catIds=catIds, iscrowd=False)
 
    # random sample the annotations
    num_annots = len(annIds)
    if num_annots < num:
        num = num_annots
        print(f"Only {num} images available...")
    annot_range = np.arange(num_annots)
    annot_sample_idx = np.random.choice(annot_range, size=num, replace=False)
    sampled_annots = annIds[annot_sample_idx]

    anns = coco.loadAnns(sampled_annots)
    
    print('\nImage Generating...')

    for i in trange(len(anns)):
        annot = anns[i]
        try:
            img = coco.loadImgs(annot["image_id"])[0]
        except Exception as e:
        ...
        # Get annotations for the image.  There should be only one since we searched earlier with iscrowd=False?
        annIds = coco.getAnnIds(imgIds=img['id'], catIds=catIds, iscrowd=None)
        anns_for_img = coco.loadAnns(annIds)
        mask = coco.annToMask(anns[0])
         
        # Generate mask
        for j in range(len(anns)):
            mask += coco.annToMask(anns_for_img[j])
